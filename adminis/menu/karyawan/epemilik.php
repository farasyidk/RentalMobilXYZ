...
$id = $_GET['id'];
$query = $db->query("select * from pemilik where IdPemilik = '$id'");
$array = $query->fetch();
?>
...
/*  $updatetfild = array('NmPemilik' => $namaL, 
  ...
  $update = $db->query("update pemilik set NmPemilik = '$namaL', alamat = '$alamat', telpon = '$telepon' where IdPemilik = '$id'");
  if ($update) {
    echo "sukses";
  } else {
    echo "string";
